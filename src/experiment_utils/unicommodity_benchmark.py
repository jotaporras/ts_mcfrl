...
def generate_hardwired_last_mile_mcf(nodes, arcs,num_customers,num_dcs,base_demand,default_capacity=10000,default_cost=1):
    base_supply = int(-base_demand * num_customers / num_dcs)
    mcf = pywrapgraph.SimpleMinCostFlow()
    for i in range(0, len(arcs)):
        mcf.AddArcWithCapacityAndUnitCost(arcs[i]['from'], arcs[i]['to'],
                                          default_capacity, default_cost)
        # Add node supplies.
...
def simple_unicommodity_min_cost_flow_benchmark(num_trials=3):
    ...
    for num_t in range(0, 3000, 250):
        ...
        for i in range(num_trials):
            ...
            mcf = generate_hardwired_last_mile_mcf(nodes, arcs, num_customers, num_dcs, base_demand)
        ...
        results.append({
            ...
            'elapsed_s': str(round_to_1(float(sum(times))/float(len(times))/1000))+"s"
        })
