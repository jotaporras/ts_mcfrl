...
def generate_basic(num_dcs, num_customers, num_t, dcs_per_customer):  # todo test
    ...
    :param num_dcs: num of nodes without costumers
    :param num_customers: num of costumers
    :param num_t: the projected duration in which the delivery reach the costumer
    :param dcs_per_customer: dcs per costumer
    """
    # Choose dcs per customer
    base_dc_assignment = np.zeros(num_dcs)
    base_dc_assignment[0:dcs_per_customer] = 1   #Rellena con 1
    ...
    dc_nodes = [{"id": dc * num_t + t, "name": f"dc_{dc}:{t}", 't': t, 'place': dc, 'type': "dc"} for dc in
                range(num_dcs) for t in range(num_t)]

    # generate customer nodes
    customer_nodes = [
        {"id": len(dc_nodes) + c, "name": f"c_{c}:{num_t - 1}", 't': num_t - 1, 'place': len(dc_nodes) + c, "type": "c"}
    ...
    dc_time_arcs = [{'name': f"dc_{dc}:{t}->dc_{dc}:{t + 1}", 'from': dc * num_t + t, 'to': dc * num_t + t + 1} for dc
                    in range(num_dcs) for t in range(num_t - 1)]

    # generate dc transport arcs, same lapse transportation, and loops allowed.
    dc_transport_arcs = [{'name': f"dc_{dc_a}:{t}->dc_{dc_b}:{t}", 'from': dc_a * num_t + t, 'to': dc_b * num_t + t} for
                         dc_a
                         in range(num_dcs) for dc_b in range(num_dcs) for t in range(num_t) if dc_a != dc_b]
...
def generate_basic_multicommodity(num_dcs, num_customers, num_t, dcs_per_customer, num_k, mean_k_demand):
    ...
    inventory_distribution = np.random.dirichlet([5.] * num_dcs,
                                                 num_k)  # (num_k,num_dc) of dc distribution of inventory.

    supply_per_dc = np.floor(inventory_distribution * demand_per_product.reshape(4, 1))
    supply_per_dc[:, 0] = supply_per_dc[:, 0] + demand_per_product - np.sum(supply_per_dc, axis=1)
