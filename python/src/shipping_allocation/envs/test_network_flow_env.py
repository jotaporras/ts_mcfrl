from envs.network_flow_env import ActualOrderGenerator, NaiveInventoryGenerator, EnvironmentParameters, \
    ShippingFacilityEnvironment, RandomAgent
...
def test_one_timestep():
    ...
    physical_network = PhysicalNetwork(num_dcs, num_customers, dcs_per_customer, demand_mean, demand_var,
                                       num_commodities)
...
def run_with_params(num_dcs, num_customers, dcs_per_customer, demand_mean, demand_var,
                    num_commodities, orders_per_day, num_episodes):
    physical_network = PhysicalNetwork(num_dcs, num_customers, dcs_per_customer, demand_mean, demand_var,
                                       num_commodities)
    # order_generator = NaiveOrderGenerator(num_dcs, num_customers, orders_per_day)
    order_generator = ActualOrderGenerator(physical_network, orders_per_day)
...
def test_sliding_ten():
    ...
    run_with_params(num_dcs,num_customers,dcs_per_customer,demand_mean,demand_var,num_commodities,orders_per_day, num_episodes)
