...
from experiment_utils.problem_generator import generate_basic_multicommodity, generate_basic
...
class ShippingPointConsolidationCSPSolver:
    ...
    def create_model(self):
        ...
        for delivery_subset_arcs in self.exclusive_arcs:
            ...
            boolean_vars_by_commodity = {} #todo this is not being used, i think it should
            ...
            for commodity in boolean_vars_by_commodity.keys(): #this is always empty. #todo see if delete.
        ...
        for commodity, to_node in boolean_vars_by_commodity_destination.keys(): #verified correct.
        ...
        for n in self.nodes:
            ...
            model.Add((-inbound_var_sum + outbound_var_sum) == n.supply)#this is the correct
...
def example2():
    ...
    # UserTime = 11.664637247
    # WallTime() = 11.664637164
    # ======
    #K=300
    # Feasible solution
    # Objective value = 9000.0
    # UserTime = 20.270405200000003
    # WallTime() = 20.270404221
    # ======
    K = 3000
    # Feasible solution
    # Objective value = 90000.0
    # UserTime = 21.133069201
    # WallTime() = 21.133070022000002

    commodities = range(1, K+1)
    # commodities = [1]
    nodes = []
    ...
    for k in commodities:
        shift=num_nodes*(k-1)
        arcs.append(Arc(i,shift + 1,shift + 2,k,1,0,0)); i+=1
        arcs.append(Arc(i,shift + 1,shift + 5,k,1,0,0)); i+=1
        arcs.append(Arc(i,shift + 2,shift + 1,k,1,0,0)); i+=1
        arcs.append(Arc(i,shift + 2,shift + 3,k,1,0,0)); i+=1
        arcs.append(Arc(i,shift + 2,shift + 5,k,1,0,0)); i+=1
        arcs.append(Arc(i,shift + 2,shift + 6,k,1,0,0)); i+=1
        arcs.append(Arc(i,shift + 3,shift + 2,k,1,0,0)); i+=1
        arcs.append(Arc(i,shift + 3,shift + 4,k,1,0,0)); i+=1
        arcs.append(Arc(i,shift + 3,shift + 5,k,1,0,0)); i+=1
        arcs.append(Arc(i,shift + 3,shift + 6,k,1,0,0)); i+=1
        arcs.append(Arc(i,shift + 4,shift + 3,k,1,0,0)); i+=1
        arcs.append(Arc(i,shift + 4,shift + 6,k,1,0,0)); i+=1
    destinations = [5,6]
    mutual_arcs = []
...
if __name__ == '__main__':
    #example1()
    example2() #todo aqui quede probar para dos commodities.
    #example3()
    #example4()
    #maybe the infinite loop screws things up?