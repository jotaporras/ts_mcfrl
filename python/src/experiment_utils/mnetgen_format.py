...
class Arc:
    def __init__(self, name: int, from_node: int, to_node: int, commodity: int, cost: float, capacity: int,
    ...
    def __repr__(self):
        return self.__str__()
    def __str__(self):
        return "Arc(%s,%s,%s,%s,%s,%s,%s)" % (self.name,self.from_node,self.to_node,self.commodity,self.cost,self.capacity,self.mutual_capacity_id)
...
class Node:
    ...
    def __str__(self):
        return "Node(%s,%s,%s)" % (
        self.node_id, self.commodity, self.supply)
...
class MnetgenFormatWriter:
    ...
    def __init__(self, nodes: List[Node], arcs: List[Arc], capacities: List[MutualCapacity]):
        self.nodes = nodes
        self.arcs = arcs
    ...
    def __nod_lines(self):
        ...
        arcs = len(self.arcs)
        capacitated = sum([1 for arc in self.arcs if arc.mutual_capacity_id != 0]) # first bundle is for uncapacitateds.
