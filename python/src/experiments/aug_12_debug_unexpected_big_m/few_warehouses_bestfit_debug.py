...
from experiment_utils import experiment_runner, reproducibility
import numpy as np
import random
...
def run_bestfit():
    ...
    runner_bestfit.run_episodes(
        num_steps, num_episodes, orders_per_day, experiment_name=f"z_debug_bestfit_few_warehouses"
    )
    print("===DONE BESTFIT===")
...
def run_agent_66_custom_agent():
    ...
    runner_bestfit = experiment_runner.create_agent_66_experiment_runner(num_dcs, num_customers, dcs_per_customer,
                                                                        demand_mean,
                                                                        demand_var, num_commodities, orders_per_day,
                                                                        num_steps)
    runner_bestfit.run_episodes(
        num_steps, num_episodes, orders_per_day, experiment_name=f"z_debug_agent_highest"
    )
    print("===DONE agent66===")
