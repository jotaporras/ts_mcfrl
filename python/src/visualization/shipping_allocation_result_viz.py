...
def read_all_summary_movement_reports(experiment_base:str) -> List[pd.DataFrame]:
    ...
    summary_movement_reports = [pd.read_csv(os.path.join(experiment_base, f"ep_{i}/summary_movement_report.csv")) for i in
                                range(max_ep + 1)]
...
def calculate_costs_per_episode(consolidated_summary,cost_col='total_cost'):
    costs_per_episode = consolidated_summary.groupby("episode")[cost_col].sum().reset_index().sort_values('episode')
...
def compare_two_runs(consolidated_summary_a, consolidated_summary_b, simplification_factor=1e9):
    # copy
...
if __name__ == '__main__':
    summary_movement_reports = read_all_summary_movement_reports("data/results/bestfit_few_warehouses")
